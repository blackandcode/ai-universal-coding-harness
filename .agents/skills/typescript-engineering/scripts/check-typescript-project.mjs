...
function findTsconfigs(dir, maxDepth = 3, depth = 0, result = []) {
  ...
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    if (
      ['node_modules', '.git', 'dist', 'build', 'coverage', '.next', '.turbo'].includes(entry.name)
    )
      continue;
...
if (tsMajor && tsMajor < 6)
  errors.push(
    `Installed/declared TypeScript appears to be ${actualTs ?? declaredTs}; this skill targets TS7 with TS6 only as a bridge.`,
  );
if (tsMajor === 6)
  info.push(
    'TypeScript 6 detected: treat this as a migration/tooling compatibility lane and validate with TS7 before declaring a TS7 migration complete.',
  );
if (tsMajor === 7)
  info.push(
    'TypeScript 7 detected: TS6-deprecated options are hard errors and legacy compiler-API consumers require explicit compatibility planning.',
  );
...
for (const file of tsconfigs) {
  ...
  if (shown?.status === 0) {
    try {
      effective = JSON.parse(shown.stdout);
    } catch {
      /* ignore */
    }
  } else if (shown) {
    errors.push(
      `${rel}: installed tsc rejects this project config: ${(shown.stderr || shown.stdout).trim().split('\n').slice(0, 8).join(' | ')}`,
    );
  }
  ...
  if (['node', 'node10', 'classic'].includes(lower(rawC.moduleResolution)))
    removed.push(`moduleResolution:${rawC.moduleResolution}`);
  if (['amd', 'umd', 'system', 'systemjs', 'none'].includes(lower(rawC.module)))
    removed.push(`module:${rawC.module}`);
  if (lower(rawC.target) === 'es5') removed.push('target:ES5');
  if (rawC.esModuleInterop === false) removed.push('esModuleInterop:false');
  if (rawC.allowSyntheticDefaultImports === false)
    removed.push('allowSyntheticDefaultImports:false');
  if (rawC.alwaysStrict === false) removed.push('alwaysStrict:false');
  if (removed.length)
    errors.push(`${rel}: TS7-incompatible/legacy options found: ${removed.join(', ')}.`);
  ...
  if (
    hasBundler &&
    !['bundler', undefined].includes(resolution) &&
    !/node(next|16)/.test(String(resolution))
  ) {
    warnings.push(
      `${rel}: bundler detected; verify moduleResolution is intentionally Bundler (or framework-required equivalent).`,
    );
  }
  if (!hasBundler && hasNodeTypes && moduleKind && !/node(next|16)/.test(String(moduleKind))) {
    warnings.push(
      `${rel}: Node types/runtime detected; verify module is NodeNext for direct Node execution rather than generic ${c.module}.`,
    );
  }
  if (
    hasReact &&
    /app|src|json$/i.test(rel) &&
    jsx &&
    !['react-jsx', 'react-jsxdev', 'preserve'].includes(jsx)
  ) {
    errors.push(`${rel}: React 19 requires the modern JSX transform; jsx=${c.jsx} is suspicious.`);
  }
  if (Array.isArray(types) && types.includes('*'))
    warnings.push(
      `${rel}: types:["*"] restores old ambient enumeration; prefer explicit global type packages in TS7.`,
    );
  if (
    hasNodeTypes &&
    Array.isArray(types) &&
    !types.includes('node') &&
    /node(next|16)/.test(String(moduleKind))
  ) {
    warnings.push(
      `${rel}: Node config appears not to include "node" in compilerOptions.types; TS7 defaults ambient types to [].`,
    );
  }
  if (
    c.noEmit !== true &&
    raw.include?.some?.((x) => String(x).includes('src')) &&
    !('rootDir' in rawC) &&
    !effective?.compilerOptions?.rootDir
  ) {
    warnings.push(
      `${rel}: source appears under src but rootDir is not explicit; TS7 defaults rootDir to the config directory.`,
    );
  }
  if (c.strict === false)
    warnings.push(`${rel}: strict is explicitly disabled; TS7 defaults strict to true.`);
  if (c.noUncheckedSideEffectImports === false)
    warnings.push(`${rel}: noUncheckedSideEffectImports is disabled; TS7 defaults it to true.`);
  if (c.verbatimModuleSyntax !== true)
    info.push(
      `${rel}: consider verbatimModuleSyntax:true so type/value import behavior matches the runtime module model.`,
    );
  if (c.noUncheckedIndexedAccess !== true)
    info.push(
      `${rel}: consider noUncheckedIndexedAccess:true for new/strict code; plan migration before enabling on a mature codebase.`,
    );
  if (c.exactOptionalPropertyTypes !== true)
    info.push(
      `${rel}: consider exactOptionalPropertyTypes:true for new code; React prop semantics may expose migrations.`,
    );
...
if (tsMajor === 7 && toolApiRisk.length) {
  warnings.push(
    `TS7 compiler-API compatibility must be verified for tooling present in this project: ${toolApiRisk.join(', ')}. TS7.0 does not expose the previous compiler API.`,
  );
}
...
if (!scripts.typecheck && !Object.values(scripts).some((v) => /\btsc\b/.test(String(v)))) {
  warnings.push(
    'No obvious TypeScript typecheck script found. A bundler/transpiler build does not replace TS7 typechecking.',
  );
}
