...
### Runtime Reviewer Role Precedence

When the orchestrator resolves settings for reviewer adapters, role configurations (`reviewer.<role>`) override global adapter settings (`harnesses.<adapter>`) on a per-field basis:
...
| ----------------- | ----------------------------------------------------------------------------------------------- |
| `model`           | `reviewer.<role>.model` → `harnesses.<adapter>.model` → static adapter default                  |
| `binary`          | `reviewer.<role>.binary` → `harnesses.<adapter>.binary` → static adapter default                |
| `reasoningEffort` | `reviewer.<role>.reasoningEffort` → `harnesses.<adapter>.reasoningEffort` → `'low'`             |
| `verbosity`       | `reviewer.<role>.verbosity` → `harnesses.<adapter>.verbosity` → `'low'`                         |
| `contextMode`     | `reviewer.<role>.contextMode` → `harnesses.<adapter>.contextMode` → `'evidence_only'`           |
| `thinking`        | `reviewer.<role>.thinking` → `harnesses.<adapter>.thinking` → `'high'`                          |
| `timeoutMinutes`  | `reviewer.<role>.timeoutMinutes` → `harnesses.<adapter>.timeoutMinutes` → `8`                   |
| `timeoutSeconds`  | `reviewer.<role>.timeoutSeconds` → `0` _(when > 0, overrides `timeoutMinutes` in milliseconds)_ |

Global `harnesses.<adapter>` blocks serve as default baselines when a reviewer role omits that specific setting. Specifying role-specific tunables under `reviewer.*` isolates parameter adjustments (for example fast low-reasoning permissions vs. deep primary reviews) without leaking into other review roles.
...
| `maxUniqueQuestionsPerStage` | number  | `25`            | Cap on distinct blocking questions per stage attempt (clamped >= 1). When exhausted, applies autonomous first-option fallback with `executor.question.budget` event emission rather than failing or blocking. |
