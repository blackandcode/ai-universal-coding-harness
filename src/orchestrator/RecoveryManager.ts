...
export class RecoveryManager {
  ...
  async recover(opts: RecoveryOptions): Promise<RecoveryResult> {
    ...
    if (!runId) {
      return { ok: false, dryRun: !isApply, runId: '', stageName: '', stageIndex: -1, details, error: 'No run found.' };
    }
    ...
    try {
      ...
      return { ok: false, dryRun: !isApply, runId, stageName: '', stageIndex: -1, details, error: `Failed to load run ${runId}: ${e.message}` };
    }

    // 2. Identify stage
    let stageIndex = -1;
    if (opts.stageName) {
      stageIndex = state.stages.findIndex(s => s.name === opts.stageName || s.name.includes(opts.stageName!));
    } else {
    ...
    if (stageIndex === -1 || !state.stages[stageIndex]) {
      return { ok: false, dryRun: !isApply, runId, stageName: opts.stageName || '', stageIndex: -1, details, error: 'Could not determine stage to recover.' };
    }
    ...
    details.push(`Current run status: ${state.status}, stage status: ${stage.status}, stage phase: ${stageState.phase || 'unknown'}`);
    ...
    if (!fs.existsSync(evidencePath)) {
      const attemptFiles = fs.readdirSync(stageDir).filter(f => /^evidence-attempt-\d+\.json$/.test(f)).sort();
    ...
    if (!fs.existsSync(evidencePath)) {
      return { ok: false, dryRun: !isApply, runId, stageName, stageIndex, details, error: `No evidence.json found in runtime or stage directory ${stageDir}.` };
    }
    ...
    try {
      ...
      return { ok: false, dryRun: !isApply, runId, stageName, stageIndex, details, error: `Invalid JSON in ${evidencePath}: ${e.message}` };
    }

    if (evidence.status !== 'PASS') {
      return { ok: false, dryRun: !isApply, runId, stageName, stageIndex, details, error: `Evidence status is ${evidence.status}, not PASS.` };
    }
    details.push(`Found evidence: status=${evidence.status}, quality_command="${evidence.quality_command}" (exit ${evidence.quality_exit_code}), git_diff_check_exit=${evidence.git_diff_check_exit_code}`);

    // 4. Reconstruct ACP observations
    const acpLogPath = path.join(stageDir, 'executor-acp.jsonl');
    if (!fs.existsSync(acpLogPath)) {
      return { ok: false, dryRun: !isApply, runId, stageName, stageIndex, details, error: `Missing ACP log: ${acpLogPath}` };
    }
    ...
    if (observations.length === 0) {
      return { ok: false, dryRun: !isApply, runId, stageName, stageIndex, details, error: `Failed to extract any command observations from ${acpLogPath}.` };
    }
    ...
    details.push('Evidence corroboration PASSED: Quality command and git diff --check verified with exit code 0.');
    ...
    if (!isApply) {
      ...
      details.push(`Would transition stage phase to "quality" with patch_fingerprint=${currentPatchFingerprint}`);
    ...
    fs.writeFileSync(observationsFile, observations.map(o => JSON.stringify(o)).join('\n') + '\n', 'utf8');
