...
export class HarnessRegistry {
  ...
  registerExecutor(id: string, f: (ctx: any) => ExecutorHarness) {
    this.executors.set(id, f);
  }
  registerReviewer(id: string, f: (ctx: any) => ReviewerHarness) {
    this.reviewers.set(id, f);
  }
  executor(id: string, ctx: any) {
  ...
  async loadConfigured(modules: string[] = CONFIG.harnessModules) {
    for (const spec of modules) await this.loadModule(spec);
  }
