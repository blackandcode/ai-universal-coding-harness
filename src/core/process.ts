...
export async function runProcess(
  ...
  return await new Promise((resolve, reject) => {
    ...
    const er = readline.createInterface({ input: child.stderr });
    er.on('line', (line: string) => {
      stderr += line + '\n';
    ...
    if (opts.timeoutMs)
      timer = setTimeout(() => {
        ...
        finish(() =>
          reject(
            new Error(`Command timed out after ${opts.timeoutMs}ms: ${cmd} ${args.join(' ')}`),
          ),
        );
      }, opts.timeoutMs);
  ...
  return await new Promise((resolve, reject) => {
    ...
    if (opts.timeoutMs)
      timer = setTimeout(() => {
        ...
        finish(() => reject(new Error(`Command timed out after ${opts.timeoutMs}ms: ${command}`)));
      }, opts.timeoutMs);
    child.on('error', (e: any) => finish(() => reject(e)));
    child.on('exit', (code: number | null) =>
      finish(() => {
