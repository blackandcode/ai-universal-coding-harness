...
import {ExecutionEvidence, CommandObservation} from '../types.js';
...
export function validateEvidence(file: string, stage: string, attempt: number) {
  ...
  let e: any;
  try { e = JSON.parse(fs.readFileSync(file, 'utf8')); }
  catch { return { ok: false, reason: 'evidence.json invalid JSON' }; }
  const ok = e.stage === stage &&
  ...
  return { ok, reason: ok ? '' : 'evidence.json missing/invalid required fields', e: e as ExecutionEvidence };
}
...
export function normalizeCommand(s: string): string {
  ...
  const shellWrap = str.match(/^(?:bash|sh|zsh|cmd\.exe|powershell)\s+(?:-c|-Command|\/c)\s+["']?([^"']+)["']?$/i);
...
export function commandMatches(observedCmd: string, targetCmd: string): boolean {
  ...
  const parts = normObs.split(/\s*(?:&&|;)\s*/).map(p => normalizeCommand(p)).filter(p => p && p !== '&&' && p !== ';');
...
export function verifyEvidenceAgainstObserved(
  e: ExecutionEvidence,
  commands: Array<CommandObservation | { command: string; exit_code: number | null; status: string; tool_id: string; sequence?: number; stage?: string; attempt?: number; quality_epoch_id?: string }>,
  ...
  if (context?.attempt != null) {
    const attemptFiltered = scopedCommands.filter(c => c.attempt == null || c.attempt === context.attempt);
  ...
  const d = [...scopedCommands].reverse().find(x => commandMatches(x.command, 'git diff --check'));
  ...
  if (!d) {
    ...
    issues.push(`git diff --check exit mismatch: evidence=${e.git_diff_check_exit_code}, ACP=${d.exit_code}`);
  }
  ...
  if (context?.last_mutation_sequence != null) {
    if (q && q.sequence != null && q.sequence <= context.last_mutation_sequence) {
      issues.push(`Quality command ran before file edits were made (mutation sequence ${context.last_mutation_sequence} >= quality sequence ${q.sequence}). Rerun quality command.`);
    }
    if (d && d.sequence != null && d.sequence <= context.last_mutation_sequence) {
      issues.push(`git diff --check ran before file edits were made (mutation sequence ${context.last_mutation_sequence} >= diff-check sequence ${d.sequence}). Rerun git diff --check.`);
    }
  }
  ...
  if (e.patch_fingerprint && context?.expected_patch_fingerprint) {
    if (e.patch_fingerprint !== context.expected_patch_fingerprint) {
      issues.push(`Evidence patch fingerprint mismatch: evidence=${e.patch_fingerprint}, current=${context.expected_patch_fingerprint}`);
    }
  }
  ...
  if (issues.length > 0) {
    const diag = commands.length === 0
      ? 'No commands were observed in session.'
      : `Observed ${commands.length} command(s) in session: ` +
        commands.slice(-8).map(c => `[${c.command} => exit: ${c.exit_code ?? 'null'}, status: ${c.status}]`).join(', ');
    issues.push(diag);
  }
