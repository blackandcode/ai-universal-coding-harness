...
import {verifyEvidenceAgainstObserved, normalizeCommand, commandMatches} from './EvidenceVerifier.js';
...
test('chained command matching recognizes components of compound command', () => {
  ...
  const r = verifyEvidenceAgainstObserved(evidence, [
    { command: 'npm run check && git diff --check', exit_code: 0, status: 'completed', tool_id: '1' }
  ]);
  assert.equal(r.ok, true);
});
